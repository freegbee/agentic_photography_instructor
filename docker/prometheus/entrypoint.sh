...
: "${MONITORING_PROMETHEUS_GPU_EXPORTER_PORT:=8000}"
# Ersetze Variablen in der Template
envsubst '\$JUROR_PORT \$IMAGE_ACQUISITION_PORT \$MLFLOW_PORT \$MONITORING_PROMETHEUS_PORT \$MONITORING_PROMETHEUS_GPU_EXPORTER_PORT' < /etc/prometheus/prometheus.yml.template > /etc/prometheus/prometheus.yml

...
from prometheus_client import CollectorRegistry, CONTENT_TYPE_LATEST, gc_collector, platform_collector, \
    process_collector
...
async def start_copy(request: AsyncImageCopyRequestV1) -> AsyncImageCopyJobResponseV1:
    ...
    job_uuid = str(uuid.uuid4())
    if request.source_dataset_id is None and request.source_directory is None:
        raise HTTPException(status_code=404,
                            detail=f"Dataset id {request.source_dataset_id} or source directory {request.source_directory} must be available.")
    ...
    try:
        new_job = ImageCopyJob(job_uuid, request.source_dataset_id, request.source_directory,
...
@v1.get("/acquisition/jobs/{job_uuid}",
        response_model=Union[AsyncImageAcquisitionJobResponseV1, AsyncImageCopyJobResponseV1])

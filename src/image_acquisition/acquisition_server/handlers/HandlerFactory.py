...
class HandlerFactory:
    ...
    def create(dataset_config: ImageDatasetConfiguration) -> AbstractHandler:
        if dataset_config.handler_type == "tar":
            return TarDownloader(dataset_config.dataset_id, dataset_config.source_url, dataset_config.calculate_images_root_path(), dataset_config.archive_root, dataset_config.target_hash)
        elif dataset_config.handler_type == "zip":
            zdl = ZipDownloader(dataset_config.dataset_id,dataset_config.source_url, dataset_config.calculate_images_root_path(), dataset_config.archive_root, dataset_config.target_hash)
            return zdl
        elif dataset_config.handler_type == "local_resource":
            lrd = LocalResourceDownloader(dataset_config.dataset_id, dataset_config.resource_file_path, dataset_config.calculate_images_root_path(), dataset_config.archive_root, dataset_config.target_hash)
            return lrd
        else:

...
TRIALS_DURATION_SECONDS = 20 * 60 * 60  # Abbruch nach 20 Stunden
TRIAL_TIMEOUT_SECONDS = int(1.5 * 60 * 60)  # Abbruch eines einzelnen Trials nach 1.5 Stunden
...
def objective(trial: optuna.Trial):
    ...
    if reward_strategy in [RewardStrategyEnum.STOP_ONLY_QUADRATIC.name, RewardStrategyEnum.STOP_ONLY_PLAIN.name]:
        success_bonus_strategy = trial.suggest_categorical("success_bonus_strategy_stop", [
            SuccessBonusStrategyEnum.FIXED.name,
            SuccessBonusStrategyEnum.SIGMOID.name,
            SuccessBonusStrategyEnum.FIXED_WITH_MDP.name,
            SuccessBonusStrategyEnum.SIGMOID_WITH_MDP.name
        ])
    else:
        success_bonus_strategy = trial.suggest_categorical("success_bonus_strategy_step", [
            SuccessBonusStrategyEnum.FIXED.name,
            SuccessBonusStrategyEnum.SIGMOID.name
        ])
    ...
    ppo_config = (PpoModelParamsBuilder(variant=PpoModelVariant.PPO_RESNET18_UNFROZEN,
                                        n_steps=N_STEPS,
    ...
    task_config = (TaskParamsBuilder(core_env=WellDefinedEnvironment.IMAGE_OPTIMIZATION,
                   ...
                   .with_rewards(strategy=RewardStrategyEnum[reward_strategy],
                                 success_bonus_strategy=SuccessBonusStrategyEnum[success_bonus_strategy],

...
class ResNet18FeatureExtractor(BaseFeaturesExtractor):
    """
        BaseFeaturesExtractor, das ein vortrainiertes ResNet18 als Feature-Backbone nutzt.
        Erwartet Eingaben im Tensorformat (B, C, H, W) mit Werten in [0,1].
        """
    ...
    def __init__(self, observation_space, features_dim: int = 512, pretrained: bool = True,
                 freeze_backbone: bool = True):
        super().__init__(observation_space, features_dim)
        self.backbone = resnet18(weights = models.ResNet18_Weights.DEFAULT if pretrained else None)

        # Backbone den classfication layer entfernen
        ...
        if freeze_backbone:
            for p in self.backbone.parameters():
                p.requires_grad = False
        self.proj = nn.Sequential(
            nn.Linear(backbone_feature_dim, 128),
            nn.ReLU(),

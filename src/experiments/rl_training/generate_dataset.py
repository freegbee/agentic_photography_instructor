...
def generate_rl_dataset():
    ...
    # Configuration
    try:
        experiment_name = input("Experiment Name [RL Dataset Generation]: ").strip()
    except EOFError:
        experiment_name = ""
    ...
    try:
        output_dir = input("Output directory [rl_training_data]: ").strip()
    except EOFError:
        output_dir = ""
    if not output_dir:
    ...
    try:
        target_dataset_id = input("Target dataset ID [rl_flickr8k]: ").strip()
    except EOFError:
        target_dataset_id = ""
    ...
    logger.info(f"Target Dataset ID: {target_dataset_id}")

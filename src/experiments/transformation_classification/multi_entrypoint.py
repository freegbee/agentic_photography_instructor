...
def select_transformers(available: List[str], min_count: int = 2) -> List[str]:
    ...
    if not selection:
        logger.warning("No selection made, using first transformers")
        return available[:min_count]
...
def entrypoint():
    """Main entrypoint for multi-transformation classification experiment."""
    print("=" * 70)
    print("Multi-Transformation Classification Experiment")
    print("=" * 70)
    print("\nThis experiment trains a model to identify multiple transformations")
    print("applied in sequence to an image (multi-label classification).")
    print("=" * 70)

    # Get experiment parameters
    try:
        experiment_name = input("\nExperiment Name (default: Multi-Transformation Classification): ").strip()
    except EOFError:
        experiment_name = ""
    if not experiment_name:
        experiment_name = "Multi-Transformation Classification"
    ...
    # For multi-transformation, we need at least 2 transformers
    ...
    print(f"\nNote: Selected {len(selected_transformers)} transformers available.")
    print(f"Each image will have N unique transformations applied in sequence.")
    try:
        num_transformations_per_image_input = input(
            f"Number of transformations per image (default: 2, max: {len(selected_transformers)}): "
        ).strip()
        num_transformations_per_image = int(num_transformations_per_image_input) if num_transformations_per_image_input else 2
        
        # Validate range
        if num_transformations_per_image < 1:
            logger.warning("Number of transformations must be at least 1, using 1")
            num_transformations_per_image = 1
        elif num_transformations_per_image > len(selected_transformers):
            logger.warning(f"Cannot apply more than {len(selected_transformers)} unique transformations, using {len(selected_transformers)}")
            num_transformations_per_image = len(selected_transformers)
    except (EOFError, ValueError):
        num_transformations_per_image = 2
    ...
    print(f"  Classification type: Multi-label (binary per transformer)")
    ...
    # Run experiment
    logger.info("Starting multi-transformation experiment with:")
    ...
    from experiments.transformation_classification.MultiTransformationClassificationExperiment import \
        MultiTransformationClassificationExperiment

    exp = MultiTransformationClassificationExperiment(
    ...
    print("  - Precision (per transformation)")
    print("  - Recall (per transformation)")
    print("  - F1 Score (harmonic mean)")
    print("  - Exact Match (all transformations correct)")
    print("\nCheck MLflow UI for results: http://localhost:5321")
    print("=" * 70)
